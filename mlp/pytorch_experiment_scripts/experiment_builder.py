...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
        ...
        self.criterion = nn.BCEWithLogitsLoss().to(self.device)  # send the loss computation to the GPU
    ...
    def run_train_iter(self, x, y):
        ...
        x, y = x.to(device=self.device), y.to(
            device=self.device)  # send data to device as torch tensors
        out = self.model.forward(x)  # forward the data in the model
        b = y.type(torch.FloatTensor)
        out = out.squeeze(dim = -1)
        loss = self.criterion(out, b.cuda())  # compute loss

        self.optimizer.zero_grad()  # set all weight grads from previous training iters to 0
        loss.backward()  # backpropagate to compute gradients for current iter loss
        grad_norm = torch.nn.utils.clip_grad_norm_(self.model.parameters(),2)

        self.optimizer.step()  # update network parameters
        out = self.sigmoid(out)
        predicted = out.data.cpu()  # get argmax of predictions
        accuracy = accuracy_score(b.data.cpu(),predicted > 0.35)  # compute accuracy
        #print(predicted)
        #print(x.data.cpu())
    ...
    def run_evaluation_iter(self, x, y):
        ...
        out = self.sigmoid(out)
        predicted = out.data.cpu()  # get argmax of predictions
        accuracy = f1_score(y.data.cpu(),predicted > 0.35)  # compute accuracy
        return loss.data, accuracy
    
    def compute_Fscore(self, x, y):
        """
        Receives the inputs and targets for the model and runs an evaluation iterations. Returns loss and accuracy metrics.
        :param x: The inputs to the model. A numpy array of shape batch_size, channels, height, width
        :param y: The targets for the model. A numpy array of shape batch_size, num_classes
        ...
        predicted = out.data.cpu()  # get argmax of predictions
        a = (y.data).cpu()
        fscore = f1_score(a,predicted > 0.35)
        accuracy = accuracy_score(a, predicted > 0.35)  # compute accuracy
    ...
    def run_experiment(self):
        ...
        for i, epoch_idx in enumerate(range(self.starting_epoch, self.num_epochs)):
            ...
            with tqdm.tqdm(total=self.train_data.num_batches) as pbar_train:  # create a progress bar for training
                for idx, (x, y) in enumerate(self.train_data):  # get data batches
                    ...
                    current_epoch_losses["train_loss"].append(loss.cpu())  # add current iter loss to the train loss list
            ...
            with tqdm.tqdm(total=self.val_data.num_batches) as pbar_val:  # create a progress bar for validation
                for x, y in self.val_data:  # get data batches
                    ...
                    current_epoch_losses["val_loss"].append(loss.cpu())  # add current iter loss to val loss list.
        ...
        current_epoch_losses = {"test_acc": [], "test_f1_score": []}  # initialize a statistics dict
        with tqdm.tqdm(total=self.test_data.num_batches) as pbar_test:  # ini a progress bar
            for x, y in self.test_data:  # sample batch
                loss, accuracy = self.compute_Fscore(x=x,
                                                          y=y)  # compute loss and accuracy by running an evaluation step
                current_epoch_losses["test_f1_score"].append(loss)  # save test loss
                current_epoch_losses["test_acc"].append(accuracy)  # save test accuracy
                pbar_test.update(1)  # update progress bar status
                pbar_test.set_description(
                    "f1_score: {:.4f}, accuracy: {:.4f}".format(loss, accuracy))  # update progress bar string output
